...
@onready var cpu_label: Label = $MarginContainer/PlayerSlot/CPULabel
...
func _ready() -> void:
	var _null: int = color_option.item_selected.connect(on_new_color);
	_null = race_option.item_selected.connect(on_new_race);
	_null = team_option.item_selected.connect(on_new_team);
	_null = ready_box.toggled.connect(on_ready_toggled)
	_null = add_cpu_button.pressed.connect(on_add_cpu);
...
func update_player_data(player_dict: Dictionary) -> void:
	if (!player_dict):
		...
		if(editable): # Enable drop down options
			...
			# Make edit options available for the player
			...
			if(player_dict[GlobalConstants.IS_CPU_KEY]):
				...
				remove_button.show();
			# If this is NOT a CPU
			else:
				ready_box.disabled = false;
				remove_button.hide();
		else:
			remove_button.hide();
			race_option.disabled = true;
			color_option.disabled = true;
			team_option.disabled = true;
			ready_box.disabled = true;

func on_new_color(index: int) ->void:
	var id: int = color_option.get_item_id(index)
	color_updated.emit(slot_num, id);

func on_new_race(index: int) ->void:
	print("did we emit this signal on this race change")
	var id: int = race_option.get_item_id(index)
	race_updated.emit(slot_num, id);

func on_add_cpu() ->void:
	add_cpu_pressed.emit(slot_num);

func on_new_team(index: int) ->void:
	var id: int = team_option.get_item_id(index);
	team_updated.emit(slot_num, id);

func on_ready_toggled(p_toggle: bool) ->void:
	ready_toggled.emit(slot_num,p_toggle);
