...
const BUILDING_TYPE: Array[int] = [GlobalConstants.BuildingType.TOWNHALL, GlobalConstants.BuildingType.DEPOT]
#const PREVIEW: Texture2D = preload(GlobalConstants.BUILDING_PLACEHOLDER_TEXTURE) #check memory leak with this ref counted bs 3/16/26
...
#resources (body mesh)
const DS_BUILT:BoxMesh = preload("uid://bgbo6jbsedpk1")
const DS_UNBUILT: BoxMesh = preload("uid://ckom4tg5rygej")
...
@export var construction_value: float = 0;
...
var cmd_dict: Dictionary[int, Dictionary] = {
	...
	2: GlobalConstants.TRAIN_DWARF_SCOUT_DICTIONARY,
	3: {},
	4: {},
	5: {},
	6: {},
...
func _ready() -> void:
	...
	if(!is_constructed):
		body_mesh.mesh = DS_UNBUILT;
		body_mesh.position.y = (DS_UNBUILT.size.y / 2);
	else:
...
func take_damage(damage_int: int, attacking_team: int) -> void:
	...
	if(died):
		var _discard:int =  entity_holder.rpc("remove_entity", get_path());
