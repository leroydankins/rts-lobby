...
@onready var command_name: Label = $CommandInfoContainer/VBoxContainer/CommandName
@onready var cost_label: Label = $CommandInfoContainer/VBoxContainer/CostLabel
@onready var command_description: Label = $CommandInfoContainer/VBoxContainer/CommandDescription
...
func update_data(p_cmd_dict: Dictionary) -> void:
	cmd_dict = p_cmd_dict
	var cmd_desc: String = cmd_dict["description"];
	var cmd_name: String = cmd_dict["name"];
	...
	if(cmd_dict.has("cost")):
		...
		cost = [];
		cost_label.hide();
	command_name.text = cmd_name;
	command_description.text = cmd_desc;
	button.disabled = false;
